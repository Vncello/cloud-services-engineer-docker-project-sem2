...
Созданы файлы для `dev` и `prod` окружения. В `dev` окружении реплики не прописаны и образы собираются _локально_. Сервисы запускаются в следующей очередности:
